...
open class AbstractKit {
    ...
    open func send(from: String?, to address: String, value: Int, feeRate: Int, sortType: TransactionDataSortType, pluginData: [UInt8: IPluginData] = [:]) throws -> FullTransaction {
        try bitcoinCore.send(from: from, to: address, value: value, feeRate: feeRate, sortType: sortType, pluginData: pluginData)
    }

    public func send(from: String?, to hash: Data, scriptType: ScriptType, value: Int, feeRate: Int, sortType: TransactionDataSortType) throws -> FullTransaction {
        try bitcoinCore.send(from: from, to: hash, scriptType: scriptType, value: value, feeRate: feeRate, sortType: sortType)
    }
    ...
    open func createRawTransaction(from: String?, to address: String, value: Int, feeRate: Int, sortType: TransactionDataSortType, pluginData: [UInt8: IPluginData] = [:]) throws -> Data {
        try bitcoinCore.createRawTransaction(from: from, to: address, value: value, feeRate: feeRate, sortType: sortType, pluginData: pluginData)
    }
    ...
    open func fee(from: String?, for value: Int, toAddress: String? = nil, feeRate: Int, pluginData: [UInt8: IPluginData] = [:]) throws -> Int {
        try bitcoinCore.fee(from: from, for: value, toAddress: toAddress, feeRate: feeRate, pluginData: pluginData)
    }

    open func maxSpendableValue(from: String?, toAddress: String? = nil, feeRate: Int, pluginData: [UInt8: IPluginData] = [:]) throws -> Int {
        try bitcoinCore.maxSpendableValue(from: from, toAddress: toAddress, feeRate: feeRate, pluginData: pluginData)
    }
