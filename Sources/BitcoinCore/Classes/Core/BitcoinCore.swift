...
public extension BitcoinCore {
    ...
    func send(from: String?, to address: String, value: Int, feeRate: Int, sortType: TransactionDataSortType, pluginData: [UInt8: IPluginData] = [:]) throws -> FullTransaction {
        guard let transactionCreator else {
            throw CoreError.readOnlyCore
        }

        return try transactionCreator.create(from: from, to: address, value: value, feeRate: feeRate, senderPay: true, sortType: sortType, pluginData: pluginData)
    }

    func send(from: String?, to hash: Data, scriptType: ScriptType, value: Int, feeRate: Int, sortType: TransactionDataSortType) throws -> FullTransaction {
        guard let transactionCreator else {
            throw CoreError.readOnlyCore
        }

        let toAddress = try addressConverter.convert(lockingScriptPayload: hash, type: scriptType)
        return try transactionCreator.create(from: from, to: toAddress.stringValue, value: value, feeRate: feeRate, senderPay: true, sortType: sortType, pluginData: [:])
    }
    ...
    func createRawTransaction(from: String?, to address: String, value: Int, feeRate: Int, sortType: TransactionDataSortType, pluginData: [UInt8: IPluginData] = [:]) throws -> Data {
        guard let transactionCreator else {
            throw CoreError.readOnlyCore
        }

        return try transactionCreator.createRawTransaction(from: from, to: address, value: value, feeRate: feeRate, senderPay: true, sortType: sortType, pluginData: pluginData)
    }
    ...
    func fee(from: String?, for value: Int, toAddress: String? = nil, feeRate: Int, pluginData: [UInt8: IPluginData] = [:]) throws -> Int {
        guard let transactionFeeCalculator else {
            throw CoreError.readOnlyCore
        }

        return try transactionFeeCalculator.fee(from: from, for: value, feeRate: feeRate, senderPay: true, toAddress: toAddress, pluginData: pluginData)
    }

    func maxSpendableValue(from: String?, toAddress: String? = nil, feeRate: Int, pluginData: [UInt8: IPluginData] = [:]) throws -> Int {
        guard let transactionFeeCalculator else {
            throw CoreError.readOnlyCore
        }

        let sendAllFee = try transactionFeeCalculator.fee(from: from, for: balance.spendable, feeRate: feeRate, senderPay: false, toAddress: toAddress, pluginData: pluginData)
